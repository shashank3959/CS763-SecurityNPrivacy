...
class L2Adversary(object):
    ...
    def __call__(self, model, inputs, targets, to_numpy=True):
        ...
        # below
        inputs = cw_runutils.make_cuda_consistent(model, inputs)[0]  # type: torch.FloatTensor
        targets = cw_runutils.make_cuda_consistent(model, targets)[0]  # type: torch.FloatTensor
    ...
    def _optimize(self, model, optimizer, inputs_tanh_var, pert_tanh_var,
        ...
        batch_loss = batch_loss_var.item()#batch_loss = batch_loss_var.data[0]  # type: float
    ...
    def _compensate_confidence(self, outputs, targets):
        ...
        if self.targeted:
            ...
            outputs_comp[rng, targets.astype('int')] -= self.confidence
        else:
            # for each image $i$:
